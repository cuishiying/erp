...
# ***********************触发器******************************
# 执行顺序：cnoa_z_wf_d_93_1276——cnoa_wf_u_step——cnoa_jxc_stock_goods_detail

DELIMITER //

# 流程发起预减 （AND NEW.stepname <> '出库确认'）。流程完成不计算价格
DROP TRIGGER IF EXISTS cnoa_afterinsert_on_cnoa_wf_u_step;
  ...
  BEGIN
    ...
    ELSEIF (NEW.stepType = 2) THEN
      IF((SELECT COUNT(1) FROM `cnoa_jxc_stock_goods_detail`WHERE tempId = NEW.uFlowId)<=0) THEN
        UPDATE cnoa_z_wf_d_93_1276 SET D_257 = D_256*D_255 WHERE uFlowId = NEW.uFlowId;
...
CREATE TRIGGER cnoa_afterdelete_on_cnoa_z_wf_d_93_1276 AFTER DELETE
  ...
  BEGIN
    ...
    SELECT SUM(sum) INTO @ds FROM cnoa_jxc_stock_goods_detail WHERE goodsId = OLD.bindid;
    SELECT SUM(quantity) INTO @dq FROM cnoa_jxc_stock_goods_detail WHERE  goodsId = OLD.bindid;
    UPDATE cnoa_jxc_goods SET price = CONVERT((@ds+OLD.D_257)/(@dq+OLD.D_255),decimal(12,2)) WHERE id = OLD.bindid;
  END;
